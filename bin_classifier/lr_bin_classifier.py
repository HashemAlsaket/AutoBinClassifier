...
class LrBinClassifier(BinClassifier):
    ...
    def get_categorical(self, data, max_categories=None, proportion_cat=0.05):
        ...
        for ft in data.columns:
            categorical_ft[ft] = (
                1.0 * data[ft].nunique() / data[ft].dropna().count() < proportion_cat
            )
        ...
        data = pd.get_dummies(
            data=data, columns=[ft for ft in data.columns if categorical_ft[ft]]
        )
    ...
    def predict(self, X):
        ...
        if not self.preds:
            ...
            if diff:
                print(
                    "WARNING: The following features were dropped due to not being included in model: "
                )
                print(diff)
